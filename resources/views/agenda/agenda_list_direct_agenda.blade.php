...
<tr>
    <td style="font-size: 12px">{{convertToNepaliNumber($loop->iteration)}}</td>
    <td style="font-size: 12px">{{convertToNepaliNumber($agenda->agenda_code)}}</td>
    <td style="font-size: 12px">{{$agenda->ministry_name}}</td>
    <td style="font-size: 12px">{{$agenda->agenda_title}}</td>
    ...
    <td style="font-size: 12px">    
        <a class="btn btn-primary" style="font-size: 12px; text-decoration: none;" target="_blank" href="/admin/print-direct-agenda/{{$agenda->id}}/report" data-toggle="tooltip" title="PDF Print">Print</a>
    </td>
    <td style="font-size: 12px">
        ...
        @endif
        <a href="#" style="font-size: 12px" class="btn btn-success btn-sm" onclick="myFunction(this)" class="fancybox" id="btn_choose_file{{$agenda->agenda_history_id}}" data-id="{{$agenda->agenda_history_id}}"  data-fancybox data-src="#uploadModal{{$agenda->agenda_history_id}}">Upload PDF</a>
        ...
        <div id="uploadModal{{$agenda->agenda_history_id}}"  style="display: none;">
            ...
            <form id="uploadForm{{$agenda->agenda_history_id}}" enctype="multipart/form-data">
                <input id="pdf_upload{{$agenda->agenda_history_id}}" type="file" required name="file_upload[{{$agenda->agenda_history_id}}]">
    ...
    function savePdf(agendaHistoryId) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $.fancybox.close();
                $('#agenda_file' + agendaHistoryId).attr('href', response.file_link);
            },
