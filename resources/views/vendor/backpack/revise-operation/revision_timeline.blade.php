    ...
    <div class="card timeline-item-wrap">
      ...
      @if($history->key == 'created_at' && !$history->old_value)
        <div class="card-header">
          ...
          {{ $history->userResponsible()?$history->userResponsible()->name:trans('revise.guest_user') }} {{ trans('revise.created_this') }} {{ $crud->entity_name }}
        </div>
      @else
        <div class="card-header">
          <strong class="time"><i class="la la-clock"></i> {{ date('h:ia', strtotime($history->created_at)) }}</strong> -
          {{ $history->userResponsible()?$history->userResponsible()->name:trans('revise.guest_user') }} {{ trans('द्वारा') }} <b>{{ $history->fieldName() }} </b> {{ trans('revise.changed_the') }} 
          <div class="card-header-actions">
            <form class="card-header-action" method="post" action="{{ url(\Request::url().'/'.$history->id.'/restore') }}">
              ...
              {{-- <button type="submit" class="btn btn-outline-danger btn-sm restore-btn" data-entry-id="{{ $entry->id }}" data-revision-id="{{ $history->id }}" onclick="onRestoreClick(event)"> --}}
                {{-- <i class="la la-undo"></i> {{ trans('revise.undo') }}</button> --}}
              </form>
          </div>
        </div>
        <div class="card-body">
          <div class="row">
            <div class="col-md-6">{{ mb_ucfirst(trans('revise.from')) }}:</div>
            <div class="col-md-6">{{ mb_ucfirst(trans('revise.to')) }}:</div>
          </div>
          <div class="row">
    ...
    function onRestoreClick(e) {
      ...
      $.ajax('{{ url(\Request::url()).'/' }}' +  revisionId + '/restore', {
        ...
        success: function(revisionTimeline) {
          ...
          new Noty({
              type: "success",
              text: "{{ trans('revise.revision_restored') }}"
          }).show();
        },
        error: function(data) {
