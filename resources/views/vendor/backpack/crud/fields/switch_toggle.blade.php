                ...
                <div class="form-check form-check-inline">
                    <label class="radio-inline" for="{{ $field['name'] }}_{{ $optionPointer }}">
                        <input data-field-toggle="{{ json_encode($field['hide_when']) }}" type="radio"
                            class="form-check-input" id="{{ $field['name'] }}_{{ $optionPointer }}"
                            name="{{ $field['name'] }}" value="{{ $value }}" @include('crud::inc.field_attributes')
                ...
                var $toggle = function($radio) {
                    ...
                    if (Object.keys($hiddenFields[$radioSet]).length) {
                        $.each($hiddenFields[$radioSet], function(idx, field) {
                            field.show();
                            resetFieldValues(field.find('input, select')); // Reset hidden field values
                        });
                    ...
                    if (typeof $hideWhen[$value] !== undefined) {
                        $.each($hideWhen[$value], function(idx, name) {
                            ...
                            if (f.length) {
                                ...
                                f.hide();
                                resetFieldValues(f.find('input, select')); // Reset hidden field values
                            }
                        });
                    }
                };
